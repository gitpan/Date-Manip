...
independent of the high level routines and give perform useful (though
much simpler) operations. They are also significantly faster than the
high level routines.
...
savings time. One way to think of these routines is working with times
...
and undef is returned in the case of any error.
