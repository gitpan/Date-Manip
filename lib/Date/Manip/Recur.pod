...
NOTATION. In the documentation however, the written out form will
...
but is not identical to it.  For more details, refer to the BASE DATES
section below.
...
These functions can also be used to look up the values.
   ...
   ($base,$actual) = $recur->base();

The base function will return both the specified base and the actual
base dates.
