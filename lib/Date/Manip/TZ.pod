...
=item Why not use existing zoneinfo files
...
=item Why not use the native tzdata files
...
=item Why store the information in so many files
...
=item The disk space seems excessive
