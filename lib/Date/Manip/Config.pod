...
savings or not. All of this is necessary in order to correctly handle
every possible date.

The daylight savings time is only used for a couple hours each year
...
function to determine the date, time, and daylight savings time
...
savings time occurs on the computer, but NOT in the Europe/Rome
...
This is ignored. With real time zone support, daylight savings time changes
