...
This takes two objects (both of which are either Date::Manip::Date
or Date::Manip::Delta objects) and creates a third object based
on those two.
...
=item Business and non-business calculations
...
=item Exact and approximate calculations
...
non-business mode calculations) and other fields (year, month, and
...
The mode of the calculation will be determined automatically if a
delta is being operated on. In the case of date-date calculations
(when no previously created delta is available to determine the
mode), the mode is supplied as an argument.

=over 4

=item Mode in date-date calculations
...
=item Mode in date-delta calculations
...
=item Mode in delta-delta calculations
...
time that has passed, including all effects of daylight savings
time. Approximate deltas ignore the affects of daylight savings time.
...
$subtract argument is easy to understand.

When working with an approximate delta however, there is some extra
meaning to the $subtract argument.

When approximate calculations are done, there is a degree of ambiguity
which leads to some unexpected results.

An example illustrates this quite well.

If you take the date Jan 4, 2000 and subtract a delta of "1 month 1
week" from it, you end up with Nov 27, 1999 (Jan 4, 2000 minus 1 month is Dec 4,
1999; minus 1 week is Nov 27, 1999). But Nov 27, 1999 plus a delta of
"1 month 1 week" is Jan 3, 2000 (Nov 27, 1999 plus 1 month is Dec 27,
1999; plus 1 week is Jan 3, 2000).
...
=item $subtract in approximate date-date calculations
...
=item $subtract in approximate date-delta calculations
