...
The $delta will also contain information about whether it is
exact, semi-exact, or approximate.
...
For non-business calculations, a day is usually (but not always) 24
hours long. Due to daylight saving time, there is some variability though,
so whenever possible, a day is actually measured as the same time on
two days (i.e. Tuesday at noon to Wednesday at noon) even if that period
is not precisely 24 hours.  For business calculations, a days length
is determined by the length of the work day and is known exactly.
