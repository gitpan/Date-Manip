...
sub dates {
   ...
   my $err = $self->_error($range_required,$start2,$end2);
   return ()  if ($err);
   ...
   if (defined($start2)) {
      if (ref($start2)) {
         return ()  if (ref($start2) ne 'Date::Manip::Date');
         return ()  if ($start2->err());
      } else {
         my $tmp = $self->new_date();
         $tmp->parse($start2);
         return ()  if ($start2->err());
         $start2 = $tmp;
      }
      ...
      $start2 = $$self{'data'}{'start'};
   }

   if (defined($end2)) {
      if (ref($end2)) {
         return ()  if (ref($end2) ne 'Date::Manip::Date');
         return ()  if ($end2->err());
      } else {
         my $tmp = $self->new_date();
         $tmp->parse($end2);
...
sub _error {
   ...
   if ((defined $start2  ||  defined $$self{'data'}{'start'})  &&
       (defined $end2    ||  defined $$self{'data'}{'end'})) {
      my $start = (defined $start2 ? $start2 : $$self{'data'}{'start'});
      my $end   = (defined $end2   ? $end2   : $$self{'data'}{'end'});
...
sub _locate_n {
   ...
   #
   # For a 'slow' date, we'll get both the start and the end at once by
   # starting at n=0 and working forwards or backwards.
   #

   if ($$self{'data'}{'slow'}) {

      # Move backwards until we have a defined date < start
      ...
      while (1) {
         my ($date,$err)  = $self->nth($n0);
         last  if (defined $date  &&  $date->cmp($start) == -1);
         $n0--;
      }
      ...
      my $n1    = 0;
      while (1) {
         my ($date,$err)  = $self->nth($n1);
         last  if (defined $date  &&  $date->cmp($end) == 1);
         $n1++;
      }

      # Now find the first and last dates in the range.

      my $n     = $n0;
      my($date,$err,$first,$last);
      while (1) {
         ($date,$err)   = $self->nth($n);
         if (defined $date  &&  $date->cmp($start) == 1) {
            $first = $n;
            last;
         }
         $n++;
      ...
      while (1) {
         ($date,$err)  = $self->nth($n);
         if (defined $date  &&  $date->cmp($end) == 1) {
            $last = $n-1;
            last;
         }
         $n++;
   ...
   #
   # Get the target date (start or end date of the range), base date,
   # and the delta.
   #
   ...
   #
   # Estimate n based on the length of the delta.
   #

   # Calculate the length of the delta.
   my $len   = $delta->printf('%sys');

   # Calculate the length of time between the base date
   # and the target date. Then get the interval date ($nn)
   # that corresponds to that.  $n is that times the
   # number of events per interval.
   my $diff  = $base->calc($date);
   my $tot   = $diff->printf('%sys');
   my $nn    = int($tot/$len);
   my $n     = $nn*$$self{'data'}{'ev_per_d'};

   #
   # Check to make sure that the Nth event exists. If it doesn't, find
   # one near it.
   #

   my ($daten,$err) = $self->nth($n);
   return undef   if ($err);

   my $i     = 1;
   while (! defined $daten) {
      ($daten,$err) = $self->nth($n+$i);
      if (defined $daten) {
         $n = $n+$i;
         last;
      }
   ...
   #
   # Make sure that $n refers to the correct date.
   #

   my $cmp = $date->cmp($daten);
   ...
   if ($op eq 'first') {
      ...
      # We're looking for the first dateN on/after date

      if ($cmp <= 0) {

         # date <= dateN
         #    decrement N until dateN < date
         ...
         while ($cmp <= 0) {
            ...
            while (1) {
               $n--;
               ($daten,$err) = $self->nth($n);
               last  if (defined($daten));
            }
            $cmp = $date->cmp($daten);
         }
      ...
      # We're looking for the last dateN on/before date
