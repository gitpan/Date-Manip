...
(year, month, day), time of day (hour, minute, second, and weeks when
appropriate), and time zone.  It doesn't work with fractional seconds.
...
an empty string is returned. Otherwise, the name of the holiday is
returned.
...
For holidays, $checktime may be passed in. If it is non-zero, the time
is checked to see if it falls within work hours.
...
=item next_business_day

=item prev_business_day
...
=item parse_date
   ...
   $err = $date->parse_date($string);

This parses a string which contains a valid date and sets the date
part of the object (but does not change the time portion).
...
All of the printf directives are available, but with a few simplifications:
   ...
   %z,%Z     will match any time zone string
...
savings time. It is ignored in all other situations. If it is
...
=item Complete vs. truncated dates and times
...
=item ISO 8601 combined date and time
...
=item Non-ISO 8601 combined date and time
...
=item Special date and time strings
...
=item Addition combined date and time formats
...
from the date.
   ...
   Minute, Second, Time zone
       ...
       %z     time zone as GMT offset  - +01:00:00
