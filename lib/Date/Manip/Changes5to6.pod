...
version 6.00. Anyone wanting to upgrade from version 5 to version 6
should read this.
...
=item Minimal time zone support
...
=item (*) Requires perl 5.10.0

In the past, I've avoided using some of the new features of perl in
order to allow Date::Manip to run on older versions of perl.  In general,
none of the new features too date would have had a major impact on
how Date::Manip was written.

However, as I was rewriting all the date parsing, the ability to do
named capture buffers allowed me to handle the complicated regular
expressions in a dramatically simpler fashion.

It was too much to turn down. Hopefully, since 5.10.0 has been out for
some time now, this will not prohibit too many people from using the
new version of Date::Manip.

Please do not request that I rewrite the parsing to be compatible with
earlier versions of perl. Although I sympathize with those people
who need to use an older version of perl, this is not something that
I'm interested in doing.

=item (*) Breaking into smaller modules
...
module from 6.00 is backwards compatible, but there are a number of
...
determining the system timezone using the curr_zone_methods function.
...
=item (*) Complete handling of time zones
...
=item Individual time zones will no longer be added

Prior to 6.00, time zones were added upon request. Since 6.00 now
...
For example, Date::Manip 6.00 already includes all zoneinfo time zones,
all time zones defined in the Microsoft Windows operating systems,
and time zones listed in RFC-822.
...
=item Nonstandard time zone abbreviations removed
...
The following time zones were in Date::Manip 5.xx but not in 6.00.
   ...
   CHST    +1000    Guam Standard, USSR Zone 9
...
=item A lot of support modules and files
...
=item (*) Date_Init handling of config variables
...
=item Date_Init doesn't return the config variables
...
At some point in the future, deprecated config variables will be
removed.
...
=item (*) Parsing is now more rigid
...
=item Support dropped for a few formats
...
=item No longer parses the Apache format
...
=item Date_PrevWorkDay behavior
...
=item Dropped mixed style delta parsing
...
=item Approximate date/date calculations
...
=item Approximate relationships in deltas
...
=item The day field meaning changed in a few recurrences
...
=item Recurrence range now inclusive
...
=item Dropped support for a couple English recurrences
...
=item Other minor recurrence changes
