...
this is the same type of attitude that caused the great Y2K problem... but
...
results, pass it in either as ParseDate("09:00:00") or
ParseDateDelta("09:00:00").
...
correct delta (ignoring daylight saving time) is returned in weeks, days,
...
In addition, the following time zone abbreviations are also accepted.  In a
few cases, the same abbreviation is used for two different time zones (for
...
=item Date::Manip fails for dates in the year 9999 or 0001.

In one section of the code, week-of-year values must be calculated which
actually involve looking into the future or past up to 1 week.  Since
Date::Manip only works for dates with a year between 0001 and 9999, the
first week of 0001 and the last week of 9999 fail.

No effort will be made to correct this because the added functionality is
simply not that important (to me), especially since the Gregorian calendar
doesn't apply in the 0001 case, and may not apply in the 9999 case (see
the section below WHAT DATES ARE DATE::MANIP USEFULE FOR?).
...
=head1 WHAT DATES ARE DATE::MANIP USEFULE FOR?
...
One caveat to this as described above is that in one part of the code
(calculating week-of-year values), Date::Manip actually uses values one
week after and one week before the date being manipulated. As such, the
first week in the year 0001 fail (because a week before is in the year 1
BC), and the last week in the year 9999 fail (because a week later is in
10,000). To be absolutely safe, I will state that Date::Manip works as
described in this manual during the years 0002 to 9998 (unless you've
encountered a bug, or are using the module incorrectly). If you submit a
"bug" using a date that is not in this range, I will not attempt to fix it
